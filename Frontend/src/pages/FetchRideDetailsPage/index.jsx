...
export default function FetchRideDetailsPage(req, res) {
 ...
 let [dataDetails,setDataDetails]=useState();
  ...
  const fetchRideDetailsPeriodically = () => {
    axios({
      method: "post",
      url: "https://trip-tide.onrender.com/api/ride/getById",
      headers: {
  ...
  // Use useEffect to start fetching ride details immediately
  useEffect(() => {
    fetchRideDetailsPeriodically();

    // Use setInterval to fetch ride details every 3 seconds
  ...
  async function submitRating() {
    ...
    axios({
      method: "post",
      url: "https://trip-tide.onrender.com/api/ride/rate",
      headers: {
        auth_token: cookies["auth_token"],
      },
      data: {
  ...
  async function driverReachedPickup() {
    axios({
      method: "post",
      url: "https://trip-tide.onrender.com/api/ride/change-status",
      headers: {
        auth_token: cookies["auth_token"],
      },
      data: {
    ...
    <div className="ride-container">
      {rideDetails && (
        <div className="ride-container-2">
          ...
          {isDriver && rideDetails.status === "ENDED" && (
            <Button onClick={BackToRide}> Ride Page</Button>
          )
